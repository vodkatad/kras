...
def traceLineage(mlineage,mutid):
    ...
    while first_mut > 0:
        trace += recent_muts
        recent_muts = mlineage[first_mut].split(',') # non mi e` chiara la relazione tra indice in mlineage e mut_id...corrispondono?
        recent_muts = [int(t) for t in recent_muts]
...
def initiateFirstDeme(maxsize,lineage,current_id):
    ...
    lineage - a list that stores the lineage information of mutations - vodka: unclear structure
    current_id - the starting mutation ID
    """
    neu_list = [str(current_id)] # ["0"] to begin with
    adv_list = []
    current_deme_size = 1
    while current_deme_size < maxsize:
        ...
        neu_list = random.sample(neu_list,neu_divcells)*2 # vodka #  *2 on a list concatenate it 2 times
        # neu_list is a random sampling from neu_list, where the decided dividing cells are selected
        # at the first run 1*0.55+1 = 1.55 -> 1
        # the only cell divides, so neu_list is ['0', '0']
        if n2 > 0:
        ...
        # now we know how many cells we have and from which cell they derived
        # we want to assign mutations occurred during this cell division
        if n1 > 0:
            ...
            # we choose from np.arange(n1) a number of muts -> the index of the cells where a mutation occurred
            # XXX in one of the daughter cells only?
            for x1 in mut_assig1.keys(): # foreach of the index of cells selected to gain a mutation
                nmut = mut_assig1[x1] # the cell that gains a mut
                new_mut1 = range(current_id+1,current_id+1+nmut) # current_id is ...boh, at first round 0 (I'd say last new mut) -> we create the ids for the new muts
                mut_str = ",".join(map(str,new_mut1)) # all the muts occurred in the generation as a comma sep string
                #if nmut > 1:
                #    for t in new_mut1:
                #        multi_events[str(t)] = mut_str
                for xn in range(0,nmut):
                    current_id += 1
                    lineage += [neu_list[x1]] # we add to our lineage every mutated cell (?), first round  ['0','0']
                neu_list[x1] = mut_str # we put all the occurred muts in this cell in its neu_list
        # same horrible duplicated code for adv
        if n2 > 0:
        ...
        # a volte capita una mutazione avvantaggiosa, fuori dal resto delle mutazioni:
        # l'ultima cellula mutata si toglie da neu_list e si mette in adv_list
        # perche` si aggiunge al lineage? XXX direi che il suo lineage gia` e` stato aggiunto, no? TODO FIXME
...
def demeGrowthFission(neu_list,adv_list,lineage,current_id,current_deme_number):
    ...
    """
    # vodka: bello il codice ripetuto
...
def seqProcessing(sp,sample_keys,mlineage,size_par,mean_depth,purity):
    ...
    purity- tumor purity
    vodka: we need to tweak it, set 100% purity and avoid ngs random sampling things (we start with the real data...we are using ddPCR...)
    or we set depth to 10000, but if n cells is 10000...
    """
...
def ddPCRProcessing(sp,sample_keys,mlineage,size_par,mean_depth,purity):
    """
    Model the random sampling process in NGS and report the sequencing allele frequencies in a sample of cells
    ...
    for key in sample_keys:
        smuts = list(sp[key].background + sp[key].advant)
        all_cur_id += smuts
    sample_size = 20000                                 # the number of cells for sequencing analysis
    sample_id = random.sample(all_cur_id,sample_size)
    id_count = Counter(sample_id)
    for x in id_count.keys():
        xlineage = traceLineage(mlineage,x)
    ...
    for x in mut_count.keys():
        ...
        if true_af > 0: # > 0.00001:                             # filter mutations with very low frequency that is not detectable by ~100X sequencing depth
            ...
            if site_depth >= 0:                        # seq depth cutoff for "calling" a mutation
                var_reads = np.random.binomial(site_depth,true_af)
                seq_af = var_reads*1.0/site_depth
                if var_reads >= 0:                      # variant reads cutof for "calling" a mutation
                    sampleAF[str(x)] = (site_depth,seq_af)
    return sampleAF
...
seq_depth=10000                        # the average sequencing depth # vodka changed to 10000
percentage = int(s_coef*100)        # the percentage form of the selection

mut_id = 0
mutlineage = ['0']                  # the lineage tracer # vodka wtf
######################################################################################
...
# vodka - ideally we need to inizialize a "fixed" first Deme with a kras mutation TODO
#print(first_neu)
#print(first_adv)
#print(mutlineage)
print("n of cells with neutral muts:%d" % len(first_neu))
print("n of cells with adv muts:%d" % len(first_adv))
print("total number of simulated cells:%d" % len(mutlineage))
print("last mut id:%d" % mut_id)

# trial and error #
# first_neu and first_adv only store the last mut occurred in a cell, lineage is needed to reconstruct its history#
#def get_nmutatedcells(muts, mut_idx):
#    found = 0
#    #print("looking for ", mut_idx)
#    #print(muts)
#    for x in muts:
#        ms = [ m for m in x.split(',') if int(m) == mut_idx ]
#        found += len(ms)
#    #print("said ", found)
#    return found
#
#nmutcells = [get_nmutatedcells(first_neu, x) for x in range(0, mut_id)]
#print("max mut cells: %d" % max(nmutcells))
#print("min mut cells: %d" % min(nmutcells))
#print("mean mut cells: %d" % np.mean(nmutcells))

#stolen code to understand traceLineage (used in sequencing results)

muts = [ x.split(',') for x in first_neu+first_adv] 
last_muts = [ x[len(x)-1] for x in muts ] 
id_count = Counter(last_muts)
#print("last muts: ", id_count)
all_mut_id = []
for x in id_count.keys():
    xlineage = traceLineage(mutlineage,x)
    #print("%d cells with last mut %s seen %s" % (id_count[x], x, xlineage))
    all_mut_id += xlineage*id_count[x]
mut_count = Counter(all_mut_id)
#print(mut_count)
nmin =  min(mut_count.values())
...
# vodka using ddPCR instead of seqProcessing (different constants that are okish with ddPCR (depths??))
maf1 = ddPCRProcessing(space,tissue1,mutlineage,2,seq_depth,1)
maf2 = ddPCRProcessing(space,tissue2,mutlineage,2,seq_depth,1)
maf3 = ddPCRProcessing(space,tissue3,mutlineage,2,seq_depth,1)
maf4 = ddPCRProcessing(space,tissue4,mutlineage,2,seq_depth,1)
maf5 = ddPCRProcessing(space,tissue5,mutlineage,2,seq_depth,1)
maf6 = ddPCRProcessing(space,tissue6,mutlineage,2,seq_depth,1)
maf7 = ddPCRProcessing(space,tissue7,mutlineage,2,seq_depth,1)
maf8 = ddPCRProcessing(space,tissue8,mutlineage,2,seq_depth,1)
...
# vodka: do we need them?
for k in range(0,npub):
    #def pubMutGenerator(n,size_par,mean_depth,purity):
    pdepth,pmaf = pubMutGenerator(8,2,seq_depth,1) 
    # vodka purity was 1 when calling seqProcessing
